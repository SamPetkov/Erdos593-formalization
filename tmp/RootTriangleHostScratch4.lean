...
namespace Erdos593
namespace TriangleHostScratchRoot4
...
def triangleHost (κ : Type u) [DecidableEq κ] : TripleSystem (Pair κ) (Triangle κ) where
  Inc p t := p.1 ⊆ t.1
  edge_ncard := by
    intro t
  ...
  simple := by
    ...
    · intro v hv
      ...
      exact (Set.ext_iff.mp htu p).mp hpt |>.apply hvp
    · have ht : t.1.card = 3 := t.2
      have hu : u.1.card = 3 := u.2
      omega

theorem triangleHost_linear (κ : Type u) [DecidableEq κ] : (triangleHost κ).Linear := by
