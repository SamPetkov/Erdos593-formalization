...
/-- Scratch canonical final index of the ordinal successor of a prefix length. -/
noncomputable def snocLast {α : TraceCarrier} (p : TracePrefix α) :
    (Order.succ p.length).ToType :=
  Ordinal.ToType.mk ⟨p.length, Order.lt_succ_iff.mpr (le_refl _)⟩
...
theorem snoc_node_last {c : TraceColoring} {α : TraceCarrier}
  ...
  simpa [TracePrefix.snocLast_toOrd] using (lt_irrefl p.length)
