...
namespace Erdos593.TripleSystem.TriangleHost.ErdosRado.TmpRankCheck3
...
theorem treeRank_eq_height (α : TraceCarrier) :
  ...
  refine (wellFounded_lt : WellFounded (fun x y : TraceCarrier => x < y)).induction
    (C := fun x => T.treeRank x = T.height x) α ?_
  intro α ih
  apply le_antisymm
...
end Erdos593.TripleSystem.TriangleHost.ErdosRado.TmpRankCheck3
