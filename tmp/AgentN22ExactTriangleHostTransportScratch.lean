...
/-!
# Scratch: exact lifted TriangleHost transport

This is a scratch-only implementation of the conditional N22 bridge.  It
relocates the base-universe triangle host into arbitrary vertex and edge
universes, pulls a canonical countable colouring back to natural colours,
and invokes the generic nonlinearity obstruction.  It does not prove the
Ramsey hypothesis.
-/
...
/-- The base-universe triangle host lifted into the ambient universes. -/
...
/-- The lifted host has exactly the original incidence relation on lifted data. -/
...
/-- A proper colouring of the exact host by the canonical countable colour
type would induce a natural-number proper colouring of the base host. -/
...
/-- The exact lifted host is uncountably chromatic under the Ramsey interface. -/
